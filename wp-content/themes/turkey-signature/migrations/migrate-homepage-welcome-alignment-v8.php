<?php
/**
 * Align the welcome copy with the collage top and widen the image column once.
 *
...
$migration_key   = 'turkey_signature_homepage_welcome_alignment_v8';
...
$v7_state = get_option( 'turkey_signature_homepage_welcome_v7' );

if ( ! is_array( $v7_state ) || (int) ( $v7_state['page_id'] ?? 0 ) !== $page_id ) {
	throw new RuntimeException( 'The welcome homepage migration v7 must exist before the alignment migration.' );
}
...
if (
	'' === $current_serialized ||
	false === strpos( $current_serialized, 'welcome-layout' ) ||
	false === strpos( $current_serialized, 'flex-basis:52%' ) ||
	false === strpos( $current_serialized, 'flex-basis:48%' ) ||
	3 !== substr_count( $current_serialized, 'class="wp-block-image size-full manifesto-image' )
) {
	throw new RuntimeException( 'The current welcome block no longer matches the expected v7 structure.' );
}
...
if ( ! is_array( $target_block ) ) {
	throw new RuntimeException( 'The aligned welcome block could not be found in the landing pattern.' );
}
...
if (
	...
	false === strpos( $target_serialized, 'flex-basis:54%' ) ||
	false === strpos( $target_serialized, 'flex-basis:46%' ) ||
	3 !== substr_count( $target_serialized, 'class="wp-block-image size-full manifesto-image' )
) {
	throw new RuntimeException( 'The generated aligned welcome block is incomplete.' );
}
...
if ( 1 !== $replacement_count ) {
	throw new RuntimeException( 'The aligned welcome block replacement count was not exactly one.' );
}
...
if ( ! $persisted_home instanceof WP_Post ) {
	throw new RuntimeException( 'The homepage could not be read after aligning the welcome section.' );
}
...
if (
	...
	false === strpos( $persisted_markup, 'flex-basis:54%' ) ||
	false === strpos( $persisted_markup, 'flex-basis:46%' )
) {
	throw new RuntimeException( 'The aligned welcome section was not persisted correctly.' );
}

<?php
/**
 * Give every tour dialog the same contact-form call to action.
 *
...
$migration_key = 'turkey_signature_homepage_tour_cta_v25';
...
$v24_state = get_option( 'turkey_signature_homepage_contact_form_v24' );

if (
	! is_array( $v24_state ) ||
	(int) ( $v24_state['page_id'] ?? 0 ) !== $page_id ||
	empty( $v24_state['content_sha256'] )
) {
	throw new RuntimeException( 'The contact form migration v24 must exist before migration v25.' );
}
...
$validate_route = static function ( string $serialized ) use ( $find_class_blocks ) {
	$route = parse_blocks( $serialized );
	$ctas  = $find_class_blocks( $route, 'tour-detail-cta' );

	if ( 8 !== count( $ctas ) ) {
		throw new RuntimeException( 'The route must contain exactly eight tour call-to-action blocks.' );
	}

	if (
		8 !== substr_count( $serialized, '>Оставить заявку<' ) ||
		0 !== substr_count( $serialized, '>Обсудить этот тур<' ) ||
		0 !== substr_count( $serialized, '>Записаться на тур<' ) ||
		8 !== substr_count( $serialized, 'href="#contact"' )
	) {
		throw new RuntimeException( 'The tour call-to-action labels or targets are invalid.' );
	}
...
$old_discuss   = substr_count( $route_markup, '>Обсудить этот тур<' );
$old_register  = substr_count( $route_markup, '>Записаться на тур<' );
$new_cta_count = substr_count( $route_markup, '>Оставить заявку<' );

if ( '' === $route_markup ) {
	throw new RuntimeException( 'The route block could not be found.' );
}

if ( 0 === $old_discuss && 0 === $old_register && 8 === $new_cta_count ) {
...
if ( ! hash_equals( (string) $v24_state['content_sha256'], $current_hash ) ) {
	throw new RuntimeException( 'The homepage changed after migration v24; migration v25 will not overwrite it.' );
}

if ( 7 !== $old_discuss || 1 !== $old_register || 0 !== $new_cta_count ) {
	throw new RuntimeException( 'The existing tour call-to-action labels could not be isolated safely.' );
}

$target_route = str_replace(
	array( '>Обсудить этот тур<', '>Записаться на тур<' ),
	'>Оставить заявку<',
	$route_markup,
	$replacement_count
);

if ( 8 !== $replacement_count ) {
	throw new RuntimeException( 'The tour call-to-action replacement count was not exactly eight.' );
}
...
if ( ! $persisted instanceof WP_Post ) {
	throw new RuntimeException( 'The homepage could not be read after migration v25.' );
}

$persisted_route = $find_anchor_block( parse_blocks( $persisted->post_content ), 'route' );

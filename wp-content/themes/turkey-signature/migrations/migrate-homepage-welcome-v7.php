<?php
/**
 * Replace only the existing editable manifesto block with the welcome layout.
 *
 * The rest of the homepage is preserved byte-for-byte so edits made after the
 * team migration remain untouched.
 *
...
$migration_key   = 'turkey_signature_homepage_welcome_v7';
...
$v6_state = get_option( 'turkey_signature_homepage_team_v6' );

if ( ! is_array( $v6_state ) || (int) ( $v6_state['page_id'] ?? 0 ) !== $page_id ) {
	throw new RuntimeException( 'The team homepage migration v6 must exist before the welcome migration.' );
}
...
if ( ! is_array( $current_block ) ) {
	throw new RuntimeException( 'The current editable impressions block could not be found.' );
}

$current_serialized = serialize_block( $current_block );

if (
	false === strpos( $current_serialized, 'manifesto-collage' ) ||
	3 !== substr_count( $current_serialized, 'class="wp-block-image size-full manifesto-image' )
) {
	throw new RuntimeException( 'The current impressions block no longer has the expected three-image structure.' );
}
...
if ( ! is_array( $target_block ) ) {
	throw new RuntimeException( 'The welcome block could not be found in the landing pattern.' );
}

$target_serialized = serialize_block( $target_block );

if (
	false === strpos( $target_serialized, 'welcome-layout' ) ||
	false === strpos( $target_serialized, 'Приветствую вас, дорогие' ) ||
	false === strpos( $target_serialized, 'href="#route"' ) ||
	3 !== substr_count( $target_serialized, 'class="wp-block-image size-full manifesto-image' )
) {
	throw new RuntimeException( 'The generated welcome block is incomplete.' );
}

if ( false !== strpos( $current_serialized, 'welcome-layout' ) ) {
	update_option(
		$migration_key,
		array(
			'version'          => 7,
...
if ( 1 !== substr_count( $current_content, $current_serialized ) ) {
	throw new RuntimeException( 'The current impressions block could not be isolated safely.' );
}
...
if ( 1 !== $replacement_count ) {
	throw new RuntimeException( 'The welcome block replacement count was not exactly one.' );
}
...
if ( ! $persisted_home instanceof WP_Post ) {
	throw new RuntimeException( 'The homepage could not be read after adding the welcome section.' );
}
...
if ( ! is_array( $persisted_block ) ) {
	throw new RuntimeException( 'The persisted welcome block could not be parsed.' );
}

$persisted_serialized = serialize_block( $persisted_block );

if (
	false === strpos( $persisted_serialized, 'welcome-layout' ) ||
	false === strpos( $persisted_serialized, 'Приветствую вас, дорогие' ) ||
	false === strpos( $persisted_serialized, 'href="#route"' )
) {
	throw new RuntimeException( 'The welcome section was not persisted correctly.' );
}

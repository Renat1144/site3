<?php
/**
 * Replace the scroll-driven destinations scene with direct carousel controls.
 *
...
$migration_key   = 'turkey_signature_homepage_destination_carousel_v11';
...
$v10_state = get_option( 'turkey_signature_homepage_welcome_fullwidth_v10' );

if ( ! is_array( $v10_state ) || (int) ( $v10_state['page_id'] ?? 0 ) !== $page_id ) {
	throw new RuntimeException( 'The welcome full-width migration v10 must exist before the destination carousel migration.' );
}
...
if (
	'' === $current_serialized ||
	false === strpos( $current_serialized, 'Продолжайте прокрутку' ) ||
	...
	throw new RuntimeException( 'The current route block no longer matches the expected scroll-driven structure.' );
}
...
if ( ! is_array( $target_block ) ) {
	throw new RuntimeException( 'The controlled destination carousel could not be found in the landing pattern.' );
}
...
if (
	...
	throw new RuntimeException( 'The generated destination carousel is incomplete.' );
}
...
if ( ! $persisted_home instanceof WP_Post ) {
	throw new RuntimeException( 'The homepage could not be read after updating the destination carousel.' );
}
...
if (
	...
	false !== strpos( $persisted_markup, 'Продолжайте прокрутку' )
) {
	throw new RuntimeException( 'The controlled destination carousel was not persisted correctly.' );
}

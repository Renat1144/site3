<?php
/**
 * Replace the demonstration testimonials with the approved traveler reviews.
 *
 * The migration changes only the reviews section, accepts the exact v34
 * homepage (or the already-current v35 state), saves one revision, and
 * verifies the persisted Gutenberg tree.
 *
 * @package TurkeySignature
 */

if ( ! defined( 'ABSPATH' ) ) {
	exit;
}

$migration_key   = 'turkey_signature_homepage_real_reviews_v35';
$migration_state = get_option( $migration_key );

if ( is_array( $migration_state ) && ! empty( $migration_state['page_id'] ) ) {
	$saved_page = get_post( (int) $migration_state['page_id'] );
	if (
		$saved_page instanceof WP_Post &&
		! empty( $migration_state['content_sha256'] ) &&
		hash_equals( (string) $migration_state['content_sha256'], hash( 'sha256', $saved_page->post_content ) )
	) {
		if ( defined( 'WP_CLI' ) && WP_CLI ) {
			WP_CLI::success( 'Homepage traveler reviews migration v35 is already complete.' );
		}
		return;
	}
}

$homepage_id = (int) get_option( 'page_on_front' );
$homepage    = $homepage_id ? get_post( $homepage_id ) : null;

if ( ! $homepage instanceof WP_Post || 'page' !== $homepage->post_type ) {
	throw new RuntimeException( 'The published WordPress front page could not be found.' );
}

$current_content = $homepage->post_content;
$current_hash    = hash( 'sha256', $current_content );
$pattern_path    = get_theme_file_path( 'patterns/turkey-landing.php' );

$v37_state = get_option( 'turkey_signature_homepage_tour_layout_v37' );
if (
	is_array( $v37_state ) &&
	! empty( $v37_state['content_sha256'] ) &&
	hash_equals( (string) $v37_state['content_sha256'], $current_hash )
) {
	if ( defined( 'WP_CLI' ) && WP_CLI ) {
		WP_CLI::success( 'Homepage traveler reviews migration v35 is superseded by v37.' );
	}
	return;
}

$v36_state = get_option( 'turkey_signature_homepage_reviews_price_v36' );
if (
	is_array( $v36_state ) &&
	! empty( $v36_state['content_sha256'] ) &&
	hash_equals( (string) $v36_state['content_sha256'], $current_hash )
) {
	if ( defined( 'WP_CLI' ) && WP_CLI ) {
		WP_CLI::success( 'Homepage traveler reviews migration v35 is superseded by v36.' );
	}
	return;
}

if ( ! is_readable( $pattern_path ) ) {
	throw new RuntimeException( 'The Turkey landing pattern could not be read.' );
}

ob_start();
include $pattern_path;
$landing_content = trim( (string) ob_get_clean() );

$find_block = static function ( array $blocks, string $class_name ) use ( &$find_block ) {
	foreach ( $blocks as $block ) {
		$classes = preg_split( '/\s+/', trim( (string) ( $block['attrs']['className'] ?? '' ) ) );
		if ( in_array( $class_name, $classes, true ) ) {
			return $block;
		}
		if ( ! empty( $block['innerBlocks'] ) ) {
			$found = $find_block( $block['innerBlocks'], $class_name );
			if ( is_array( $found ) ) {
				return $found;
			}
		}
	}
	return null;
};

$count_class = static function ( array $blocks, string $class_name ) use ( &$count_class ): int {
	$count = 0;
	foreach ( $blocks as $block ) {
		$classes = preg_split( '/\s+/', trim( (string) ( $block['attrs']['className'] ?? '' ) ) );
		if ( in_array( $class_name, $classes, true ) ) {
			++$count;
		}
		if ( ! empty( $block['innerBlocks'] ) ) {
			$count += $count_class( $block['innerBlocks'], $class_name );
		}
	}
	return $count;
};

$current_reviews = $find_block( parse_blocks( $current_content ), 'reviews-section' );
$target_reviews  = $find_block( parse_blocks( $landing_content ), 'reviews-section' );

if ( ! is_array( $current_reviews ) || ! is_array( $target_reviews ) ) {
	throw new RuntimeException( 'The current or target reviews section could not be isolated.' );
}

$current_reviews_markup = serialize_block( $current_reviews );
$target_reviews_markup  = serialize_block( $target_reviews );
$already_target         = hash_equals( hash( 'sha256', $current_reviews_markup ), hash( 'sha256', $target_reviews_markup ) );
$target_content         = $current_content;

if ( ! $already_target ) {
	$v34_state = get_option( 'turkey_signature_homepage_program_days_v34' );
	$v34_hash  = is_array( $v34_state ) ? (string) ( $v34_state['content_sha256'] ?? '' ) : '';
	if ( '' === $v34_hash || ! hash_equals( $v34_hash, $current_hash ) ) {
		throw new RuntimeException( 'The homepage is neither the exact v34 state nor the current v35 landing; migration v35 will not overwrite it.' );
	}
	if ( 1 !== substr_count( $target_content, $current_reviews_markup ) ) {
		throw new RuntimeException( 'The current reviews section was not unique.' );
	}
	$target_content = str_replace( $current_reviews_markup, $target_reviews_markup, $target_content, $replacement_count );
	if ( 1 !== $replacement_count ) {
		throw new RuntimeException( 'The reviews section was not replaced exactly once.' );
	}
}

$generated_blocks = parse_blocks( $target_content );
$review_names     = array( 'Алла', 'Надежда', 'Валентина', 'Татьяна' );

if (
	4 !== $count_class( $generated_blocks, 'review-card' ) ||
	1 !== $count_class( $generated_blocks, 'reviews-grid-real' ) ||
	1 !== $count_class( $generated_blocks, 'reviews-intro' ) ||
	0 !== $count_class( $generated_blocks, 'reviews-disclaimer' ) ||
	5 !== $count_class( $generated_blocks, 'program-day' ) ||
	7 !== $count_class( $generated_blocks, 'destination-card' ) ||
	7 !== $count_class( $generated_blocks, 'tour-details-trigger' ) ||
	1 !== $count_class( $generated_blocks, 'tour-selection-cta' ) ||
	false !== strpos( $target_content, 'демонстрационный текст' ) ||
	false !== strpos( $target_content, 'До публикации замените примеры' ) ||
	false !== strpos( $target_content, '"lock"' ) ||
	false !== strpos( $target_content, '"templateLock"' )
) {
	throw new RuntimeException( 'The v35 homepage structure failed validation.' );
}

foreach ( $review_names as $review_name ) {
	if ( 1 !== substr_count( $target_content, '>' . $review_name . '</p>' ) ) {
		throw new RuntimeException( sprintf( 'The v35 review author %s is missing or duplicated.', $review_name ) );
	}
}

if ( ! $already_target ) {
	wp_save_post_revision( $homepage_id );
	$updated_id = wp_update_post(
		array(
			'ID'           => $homepage_id,
			'post_content' => wp_slash( $target_content ),
		),
		true
	);
	if ( is_wp_error( $updated_id ) ) {
		throw new RuntimeException( $updated_id->get_error_message() );
	}
}

clean_post_cache( $homepage_id );
$persisted_homepage = get_post( $homepage_id );

if (
	! $persisted_homepage instanceof WP_Post ||
	! hash_equals( hash( 'sha256', $target_content ), hash( 'sha256', $persisted_homepage->post_content ) )
) {
	throw new RuntimeException( 'The v35 homepage was not persisted exactly.' );
}

update_option(
	$migration_key,
	array(
		'version'          => 35,
		'page_id'          => $homepage_id,
		'previous_sha256'  => $current_hash,
		'content_sha256'   => hash( 'sha256', $persisted_homepage->post_content ),
		'completed_at_gmt' => gmdate( 'c' ),
	),
	false
);

if ( defined( 'WP_CLI' ) && WP_CLI ) {
	WP_CLI::success( sprintf( 'Applied homepage traveler reviews v35 on page %d.', $homepage_id ) );
}

<?php
/**
 * Apply the v32 homepage composition and transition polish.
 *
 * The migration accepts only the exact v31 homepage (or the already-current
...
$migration_key   = 'turkey_signature_homepage_visual_polish_v32';
...
if ( is_array( $migration_state ) && ! empty( $migration_state['page_id'] ) ) {
	...
	if (
		...
		if ( defined( 'WP_CLI' ) && WP_CLI ) {
			WP_CLI::success( 'Homepage visual polish migration v32 is already complete.' );
		}
		return;
	}
}
...
if ( ! $already_target ) {
	...
	if ( '' === $v31_hash || ! hash_equals( $v31_hash, $current_hash ) ) {
		throw new RuntimeException( 'The homepage is neither the exact v31 state nor the current v32 landing; migration v32 will not overwrite it.' );
	}
}
...
if (
	0 !== $count_class( $generated_blocks, 'hero-actions' ) ||
	0 !== $count_class( $generated_blocks, 'hero-assurance' ) ||
	1 !== $count_class( $generated_blocks, 'hero-copy' ) ||
	...
	false === strpos( $target_content, 'Авторские туры' ) ||
	false !== strpos( $target_content, 'Оставьте контакты — поможем выбрать подходящий маршрут' ) ||
	false !== strpos( $target_content, '"lock"' ) ||
	false !== strpos( $target_content, '"templateLock"' )
) {
	throw new RuntimeException( 'The v32 homepage structure failed validation.' );
}
...
if (
	...
	throw new RuntimeException( 'The v32 homepage was not persisted exactly.' );
}
...
if ( defined( 'WP_CLI' ) && WP_CLI ) {
	WP_CLI::success( sprintf( 'Applied homepage visual polish v32 on page %d.', $homepage_id ) );
}

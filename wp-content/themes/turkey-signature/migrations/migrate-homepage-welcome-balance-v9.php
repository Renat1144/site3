<?php
/**
 * Widen the welcome collage and balance the copy column once.
 *
...
$migration_key   = 'turkey_signature_homepage_welcome_balance_v9';
...
$v8_state = get_option( 'turkey_signature_homepage_welcome_alignment_v8' );

if ( ! is_array( $v8_state ) || (int) ( $v8_state['page_id'] ?? 0 ) !== $page_id ) {
	throw new RuntimeException( 'The welcome alignment migration v8 must exist before the balance migration.' );
}
...
if (
	...
	false === strpos( $current_serialized, 'flex-basis:54%' ) ||
	false === strpos( $current_serialized, 'flex-basis:46%' ) ||
	3 !== substr_count( $current_serialized, 'class="wp-block-image size-full manifesto-image' )
) {
	throw new RuntimeException( 'The current welcome block no longer matches the expected v8 structure.' );
}
...
if ( ! is_array( $target_block ) ) {
	throw new RuntimeException( 'The balanced welcome block could not be found in the landing pattern.' );
}
...
if (
	...
	false === strpos( $target_serialized, 'flex-basis:58%' ) ||
	false === strpos( $target_serialized, 'flex-basis:42%' ) ||
	3 !== substr_count( $target_serialized, 'class="wp-block-image size-full manifesto-image' )
) {
	throw new RuntimeException( 'The generated balanced welcome block is incomplete.' );
}
...
if ( 1 !== $replacement_count ) {
	throw new RuntimeException( 'The balanced welcome block replacement count was not exactly one.' );
}
...
if ( ! $persisted_home instanceof WP_Post ) {
	throw new RuntimeException( 'The homepage could not be read after balancing the welcome section.' );
}
...
if (
	...
	false === strpos( $persisted_markup, 'flex-basis:58%' ) ||
	false === strpos( $persisted_markup, 'flex-basis:42%' )
) {
	throw new RuntimeException( 'The balanced welcome section was not persisted correctly.' );
}

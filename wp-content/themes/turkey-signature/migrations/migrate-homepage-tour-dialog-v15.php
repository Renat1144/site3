<?php
/**
 * Turn the destination cards into editable tours with detailed dialog content.
 *
...
$migration_key   = 'turkey_signature_homepage_tour_dialog_v15';
...
$v14_state = get_option( 'turkey_signature_homepage_destination_title_v14' );

if ( ! is_array( $v14_state ) || (int) ( $v14_state['page_id'] ?? 0 ) !== $page_id ) {
	throw new RuntimeException( 'The destination title migration v14 must exist before migration v15.' );
}
...
if (
	...
	false !== strpos( $current_serialized, 'tour-details-trigger' ) ||
	false !== strpos( $current_serialized, 'tour-details-library' ) ||
	4 !== substr_count( $current_serialized, 'class="wp-block-group destination-card"' )
) {
	throw new RuntimeException( 'The current route block no longer matches the expected v14 structure.' );
}
...
if (
	...
	4 !== substr_count( $target_serialized, 'class="wp-block-buttons tour-details-trigger"' ) ||
	1 !== substr_count( $target_serialized, 'class="wp-block-group tour-details-library"' ) ||
	4 !== substr_count( $target_serialized, 'class="wp-block-group tour-details"' ) ||
	4 !== substr_count( $target_serialized, '>Подробнее<' ) ||
	4 !== substr_count( $target_serialized, '>Обсудить этот тур<' )
) {
	throw new RuntimeException( 'The generated v15 tour dialog block is incomplete.' );
}
...
if ( ! $persisted_home instanceof WP_Post ) {
	throw new RuntimeException( 'The homepage could not be read after migration v15.' );
}
...
if (
	4 !== substr_count( $persisted_markup, 'class="wp-block-buttons tour-details-trigger"' ) ||
	1 !== substr_count( $persisted_markup, 'class="wp-block-group tour-details-library"' ) ||
	4 !== substr_count( $persisted_markup, 'class="wp-block-group tour-details"' ) ||
	4 !== substr_count( $persisted_markup, '>Подробнее<' )
) {
	throw new RuntimeException( 'The v15 tour dialog content was not persisted correctly.' );
}

<?php
/**
 * Move the destination controls above the cards and remove the visual hint.
 *
 * Only the existing editable route block is replaced; all other homepage
 * content remains byte-for-byte unchanged. Autoplay is a progressive
 * enhancement supplied by the shared public script.
 *
...
$migration_key   = 'turkey_signature_homepage_destination_autoplay_v12';
...
$v11_state = get_option( 'turkey_signature_homepage_destination_carousel_v11' );

if ( ! is_array( $v11_state ) || (int) ( $v11_state['page_id'] ?? 0 ) !== $page_id ) {
	throw new RuntimeException( 'The destination carousel migration v11 must exist before migration v12.' );
}
...
$current_controls   = strpos( $current_serialized, 'destination-progress' );
$current_viewport   = strpos( $current_serialized, 'destination-viewport' );

if (
	'' === $current_serialized ||
	false === strpos( $current_serialized, 'Выберите направление' ) ||
	false === strpos( $current_serialized, 'destination-slider' ) ||
	false === $current_controls ||
	false === $current_viewport ||
	$current_controls < $current_viewport ||
	4 !== substr_count( $current_serialized, 'class="wp-block-group destination-card"' )
) {
	throw new RuntimeException( 'The current route block no longer matches the expected v11 carousel structure.' );
}
...
if ( ! is_array( $target_block ) ) {
	throw new RuntimeException( 'The updated destination carousel could not be found in the landing pattern.' );
}
...
if (
	false !== strpos( $target_serialized, 'Выберите направление' ) ||
	...
	throw new RuntimeException( 'The generated v12 destination carousel is incomplete.' );
}
...
if ( ! $persisted_home instanceof WP_Post ) {
	throw new RuntimeException( 'The homepage could not be read after migration v12.' );
}
...
$persisted_controls = strpos( $persisted_markup, 'destination-progress' );
$persisted_viewport = strpos( $persisted_markup, 'destination-viewport' );

if (
	false !== strpos( $persisted_markup, 'Выберите направление' ) ||
	false === strpos( $persisted_markup, 'destination-slider' ) ||
	false === $persisted_controls ||
	false === $persisted_viewport ||
	$persisted_controls > $persisted_viewport
) {
	throw new RuntimeException( 'The v12 destination carousel was not persisted correctly.' );
}

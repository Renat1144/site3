<?php
/**
 * Add the supplied Istanbul and Cappadocia comfort tour.
 *
 * The migration replaces only the editable route block after verifying the
 * exact v19 homepage hash and saves a WordPress revision before the update.
 *
...
$migration_key   = 'turkey_signature_homepage_cappadocia_comfort_v20';
...
$v19_state = get_option( 'turkey_signature_homepage_welcome_highlight_v19' );

if (
	! is_array( $v19_state ) ||
	(int) ( $v19_state['page_id'] ?? 0 ) !== $page_id ||
	empty( $v19_state['content_sha256'] )
) {
	throw new RuntimeException( 'The welcome highlight migration v19 must exist before migration v20.' );
}
...
if ( ! hash_equals( (string) $v19_state['content_sha256'], $current_hash ) ) {
	throw new RuntimeException( 'The homepage changed after migration v19; migration v20 will not overwrite it.' );
}
...
if (
	...
	1 !== substr_count( $current_serialized, 'class="wp-block-group tour-detail-program"' ) ||
	5 !== substr_count( $current_serialized, 'class="wp-block-group tour-detail-day"' ) ||
	false === strpos( $current_serialized, '102 904 ₽' )
) {
	throw new RuntimeException( 'The current route block no longer matches the expected v19 structure.' );
}
...
if (
	'' === $target_serialized ||
	6 !== substr_count( $target_serialized, 'class="wp-block-group destination-card tour-card-' ) ||
	6 !== substr_count( $target_serialized, 'class="wp-block-buttons tour-details-trigger"' ) ||
	6 !== substr_count( $target_serialized, 'class="wp-block-group tour-details"' ) ||
	6 !== count( $target_copies ) ||
	1 !== count( $target_programs ) ||
	5 !== count( $target_days ) ||
	1 !== count( $target_prices ) ||
	2 !== substr_count( $target_serialized, 'tour-card-cappadocia-comfort' ) ||
	2 !== substr_count( $target_serialized, 'Стамбул и Каппадокия: комфорт-тур' ) ||
	1 !== substr_count( $target_serialized, 'kapadokia.png' ) ||
	false === strpos( $target_serialized, 'Каппадокия — одно из тех редких мест' ) ||
	false === strpos( $target_serialized, 'частная яхта и камерные рестораны' ) ||
	false === strpos( $target_serialized, 'полёт на воздушном шаре' ) ||
	false === strpos( $target_serialized, 'гармонию противоположностей' ) ||
	false === strpos( $target_serialized, 'Комфорт · уникальное жильё' ) ||
	false === strpos( $target_serialized, '102 904 ₽' )
) {
	throw new RuntimeException( 'The generated v20 route block is incomplete.' );
}
...
foreach ( $target_copies as $target_copy ) {
	if (
		...
		throw new RuntimeException( 'A generated v20 card contains content other than its title.' );
	}
}

if ( false !== strpos( $target_serialized, '"lock"' ) || false !== strpos( $target_serialized, '"templateLock"' ) ) {
	throw new RuntimeException( 'The generated v20 block unexpectedly contains editing locks.' );
}
...
if ( ! $persisted_home instanceof WP_Post ) {
	throw new RuntimeException( 'The homepage could not be read after migration v20.' );
}
...
$persisted_cards    = is_array( $persisted_block ) ? $find_class_blocks( array( $persisted_block ), 'destination-card-copy' ) : array();
$persisted_details  = is_array( $persisted_block ) ? $find_class_blocks( array( $persisted_block ), 'tour-details' ) : array();
...
if (
	6 !== count( $persisted_cards ) ||
	6 !== count( $persisted_details ) ||
	1 !== count( $persisted_programs ) ||
	5 !== count( $persisted_days ) ||
	2 !== substr_count( $persisted_markup, 'tour-card-cappadocia-comfort' ) ||
	2 !== substr_count( $persisted_markup, 'Стамбул и Каппадокия: комфорт-тур' ) ||
	1 !== substr_count( $persisted_markup, 'kapadokia.png' ) ||
	false === strpos( $persisted_markup, 'гармонию противоположностей' ) ||
	false !== strpos( $persisted_markup, '"lock"' ) ||
	false !== strpos( $persisted_markup, '"templateLock"' )
) {
	throw new RuntimeException( 'The v20 route block was not persisted correctly.' );
}
